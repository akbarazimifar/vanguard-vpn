...
package com.vanguard.vpn.tunnel.vpn;
...
public class Tunnel
{
	...
	{
		StringBuilder routeMessage = new StringBuilder("Rotas: ");
		...
		try
		{
			...
			tunFd = builder
					...
					.setConfigureIntent(SocksDNSService.getGraphPendingIntent(mHostService.getContext()))
					.establish();
	...
	private boolean routeThroughTunnel(final String socksServerAddress, final String[] dnsResolver, boolean forwardDns, final String udpResolver, final boolean transparentDns)
	{
		...
		if (forwardDns) {
			...
			mPdnsd.setOnPdnsdListener(new Pdnsd.OnPdnsdListener(){
				@Override
				public void onStart(){
					mHostService.onDiagnosticMessage("DNS Encaminhamento Iniciado");
				}
				@Override
				public void onStop(){
					mHostService.onDiagnosticMessage("DNS Encaminhamento Parado");
					stop();
				}
			});
	...
	private void stopVpn()
	{
		...
		if (tunFd != null)
		{
			try
			{
				mHostService.onDiagnosticMessage("<strong>Disconnected</strong>");
				tunFd.close();
			}
			catch (IOException e) {}
		}
